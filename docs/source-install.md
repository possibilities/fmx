...
The installer runs a frozen Bun dependency install, links `fmx` and `fmx-mcp`
from the checkout, builds the exact Fx Integration commit in `fx.json` as
`fmx-fx`, builds the exact Companion commit in `companion.json` as `fmx-zmx`,
and runs `fmx doctor`. By default the native commands go to `~/.local/bin` and
Bun's editable links go to `${BUN_INSTALL:-$HOME/.bun}/bin`.
