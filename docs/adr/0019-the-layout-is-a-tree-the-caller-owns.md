...
different size and there is no resize verb. Geometry is smolmux's own: every Pane
...
sprawls with the second, and it forces smolmux to know what each surface is for.
One tree means a new arrangement is a new tree, zoom is just a tree where one
Pane takes the remainder, and smolmux never learns the word "panel".
...
Before anyone applies a Layout there is nothing for smolmux to draw but its own,
