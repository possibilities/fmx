...
test("answers a request with a correlated response", async () => {
  ...
  expect(await client.call("session.list")).toEqual({ echoed: "session.list" })
})
...
test("validates the method and its params from the contract", async () => {
  ...
  await expect(client.call("session.create", { name: "tray" })).rejects.toThrow(/argv|cwd/u)
  await expect(client.call("session.create", { name: "Tray", argv: ["/bin/sh"], cwd: "/" })).rejects.toMatchObject({
    code: "invalid_params",
  })
})
