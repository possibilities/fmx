...
import { identityFor, InstanceManifest, type ManifestEntry } from "../src/instance-manifest.ts"
import { ownedInstanceId, ownershipLabels, reconcile, reconcileInstances } from "../src/instance-reconcile.ts"
...
const entry = (instanceId: string, phase: ManifestEntry["phase"] = "running"): ManifestEntry => ({
  ...identityFor(instanceId),
  displayId: 1,
  cwd: "/work",
  fxPath: "/fx",
  fxArgs: [],
...
const session = (
  ...
  command: ["/fx", "--x"],
  cwd: "/work",
  labels: state === "live" || state === "exited" ? ownershipLabels(HOME, instanceId) : {},
...
test("ownership needs every label and the name to agree", () => {
  expect(ownedInstanceId(session(ID_A), HOME)).toBe(ID_A)
  expect(ownedInstanceId(session(ID_A, "live", { labels: { ...ownershipLabels("other", ID_A) } }), HOME)).toBeNull()
  expect(ownedInstanceId(session(ID_A, "live", { labels: { ...ownershipLabels(HOME, ID_A), owner: "me" } }), HOME)).toBeNull()
  expect(ownedInstanceId(session(ID_A, "live", { name: "fmx-renamed" }), HOME)).toBeNull()
  expect(ownedInstanceId(session(ID_A, "live", { labels: { ...ownershipLabels(HOME, ID_A), pane: "p_1" } }), HOME)).toBeNull()
  expect(ownedInstanceId(session(ID_A, "live", { labels: {} }), HOME)).toBeNull()
})
...
test("every crash-window combination lands in one bucket", () => {
  ...
  expect(plan.attach.map((item) => item.entry.instanceId)).toEqual([ID_A])
  expect(plan.remove.map((item) => [item.entry.instanceId, item.session?.state ?? null])).toEqual([
    [ID_B, "exited"],
    [ID_C, null],
  ])
...
test("a live owned session nobody wrote down is adopted; an entry with no session is removed", () => {
  ...
  expect(plan.remove.map((item) => item.entry.instanceId)).toEqual([ID_A])
  expect(plan.adopt.map((item) => item.instanceId)).toEqual([ID_B])
})
...
test("refused and unreachable are unresolved, never removed, whether or not there is an entry", () => {
  ...
  expect(plan.unresolved.map((item) => [item.entry?.instanceId ?? null, item.session.name])).toEqual([
...
test("reconcileInstances applies the join: adopts, removes, consumes exit records, and seeds the registry's facts", async () => {
  ...
  try {
    ...
    expect(manifest.entries.map((item) => item.instanceId).sort()).toEqual([ID_A, ID_C])
  } finally {
    await rm(dir, { recursive: true, force: true })
  }
})

test("reconcileInstances waits for a refused session to settle, then decides", async () => {
  const dir = await mkdtemp("/tmp/fmx-reconcile-test-")
  try {
    const manifest = await InstanceManifest.open(join(dir, "m.json"), HOME)
    await manifest.beginCreate({ cwd: "/work", fxPath: "/fx", fxArgs: [], createdAt: 0, identity: identityFor(ID_A) })
    ...
    const outcome = await reconcileInstances(manifest, companion, { settleMs: 2000 })
    expect(calls()).toBe(3)
